...
void TFT_Init(void)
{
	...
	for(i=5000; i>0; i--);
	 ...
	 TFT_WriteCmd(0x0001);TFT_WriteData(0x011B);  	//011b
	 TFT_WriteCmd(0x0002);TFT_WriteData(0x0700);  
	 TFT_WriteCmd(0x0003);TFT_WriteData(0x1030);   //1030
...
void TFT_SetWindow(uint xStart, uint yStart, uint xEnd, uint yEnd)
{
	 ...
	 /*TFT_WriteCmd(0x44);TFT_WriteData(xStart);			//水平
	 TFT_WriteCmd(0x44);TFT_WriteData(xEnd <<8);		//水平
	    
	 TFT_WriteCmd(0x45);TFT_WriteData(yStart);			//垂直
	 TFT_WriteCmd(0x45);TFT_WriteData(yEnd <<8);		//垂直

	 TFT_WriteCmd(0x20);TFT_WriteData(xStart&0x00ff);   //xStart
 	 TFT_WriteCmd(0x21);TFT_WriteData(yStart<<8);   	//yStart
	 */
	 TFT_WriteCmd(0x22); 
}
